...
class VEGASMultiStratification:
    """
        多个 VEGASStratification 一起vectorize 操作
    """
    ...
    def accumulate_weight(self, nevals, weight_all_cubes):
        ...
        Args:
            ...
            也就是说这个得是按cube的indices从小到大排列的才行
        ...
        # anp.meshgrid(*([grid_1d] * self.dim), indexing="xy", like=self.backend)
        # 拿torch.meshgrid 应该很容易实现之前自己repeat的结果
        # TODO: 可能可以改一下 现在先放弃了 就拿repeat吧
    ...
    def update_DH(self):
        ...
        d_sum = anp.sum(self.dh, axis=1)
        # set the dh that has 0 d_sum to be 1
        self.dh[d_sum == 0, :] = 1. / self.N_cubes

        d_sum[d_sum == 0] = 1
        d_sum = d_sum.view(-1, 1)
    ...
    def get_NH(self, nevals_exp):
        ...
        """ 目标： 让nh.sum(axis=1) 都相等
            默认都等于 MAX(nh.sum(axis=1)
        """
        ...
        delta_nevals = max_nevals - cur_nevals # 每个integrator 差的数量
        """ 感觉这个稍微慢一点 应该也没关系吧 """
