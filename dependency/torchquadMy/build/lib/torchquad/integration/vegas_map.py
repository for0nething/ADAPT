...
class VEGASMap:
    ...
    def __init__(self, N_intervals, dim, backend, dtype, alpha=0.5) -> None:
        ...
        """ todo: change the initialization method here!"""
        self.dx_edges = (
    ...
    """ new helper function 1: set   dx_edges, x_edges by passing params to it """
    def set_map_edges(self, dx_edges, x_edges):
        # print("x_edges shape is ", x_edges.shape)
        if dx_edges.shape == self.dx_edges.shape and x_edges.shape == self.x_edges.shape:
            self.dx_edges = dx_edges
            self.x_edges = x_edges
        else:
            raise ValueError("Appointed edges should have the same number of intervals and dimensions!")
        pass


    def get_Y(self, x):
        """Get mapped sampling points in Y space

            Args:
                (n, dim)
                x (backend tensor): Randomly sampled location(s)

            Returns:
                backend tensor: Mapped points in Y space.


            step 1: 直接用torch.searchsorted
        """

        ret_Y = torch.empty_like(x)
        for d in range(self.dim):
            # iy = torch.searchsorted(self.x_edges[d, :], x[:, d], side='right')
            contigious_x = x[:, d].contiguous()

            iy = torch.searchsorted(self.x_edges[d, :], contigious_x, side='right')
            """ todo:(done) may need to transform to vectorized implementation """
            # for i in range(x.shape[0]):
            #     if iy[i] > 0 and iy[i] <= self.N_intervals:
            #         iyi = iy[i] - 1
            #         ret_Y[i, d] = (iyi + (x[i, d] - self.x_edges[d, iyi]) / self.dx_edges[d, iyi]) / self.N_intervals
            #     elif iy[i] <=0:
            #         ret_Y[i, d] = 0.
            #     elif iy[i] > self.N_intervals:
            #         ret_Y[i, d] = 1.0
            ...
            ret_Y[in_range_iyi, d] = (tmp +
                                      (x[in_range_iyi, d] -
                                      self.x_edges[d, tmp]) / self.dx_edges[d, tmp])/self.N_intervals
