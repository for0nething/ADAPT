...
import distrax
from nets import PosMulResidualBlock
    ...
    def setup(self):
        self.conditioner = nn.Sequential(
            [
                ...
                # # 【PosPlusResidualBlock】
                PosMulResidualBlock(list(self.hidden_size), loc_alpha=self.loc_alpha, pre_masks=self.pre_masks),
                # nn.relu,    # new
                PosMulResidualBlock(list(self.hidden_size), loc_alpha=self.loc_alpha, pre_masks=self.pre_masks),
...
class PosMulRQSpline(nn.Module):
    ...
    def setup(self):
        ...
        for i in range(2):
            pre_masks.append(
                sin_pe_func(pe_op="mul", pe_t=1.0, pe_alpha=self.loc_alpha,
